...
from .models import JobPost,Education,Skills_job,Apply_Job
...
class DashboardView(View):
    ...
    def get(self, request, *args, **kwargs):
        total_job_posts = JobPost.objects.filter(user=request.user).count()
        applied_users_count = sum(Apply_Job.objects.filter(job=job_post).count() for job_post in JobPost.objects.filter(user=request.user))
        accepted_applications_count = Apply_Job.objects.filter(job__user=request.user, status='Accepted').count()
        rejected_applications_count = Apply_Job.objects.filter(job__user=request.user, status='Rejected').count()
        pending_applications_count = Apply_Job.objects.filter(job__user=request.user, status='Pending').count()
        total_job_posts_html = render_to_string('Dashboard/count_total_applied.html', {'total_job_posts': total_job_posts})
        # applied_users_count_html = render_to_string('Dashboard/Accepted_application.html', {'applied_users_count': applied_users_count})
        accepted_applications_count_html = render_to_string('Dashboard/Accepted_application.html', {'accepted_applications_count': accepted_applications_count})
        # rejected_applications_count_html = render_to_string('partials/rejected_applications_count.html', {'rejected_applications_count': rejected_applications_count})
        # pending_applications_count_html = render_to_string('partials/pending_applications_count.html', {'pending_applications_count': pending_applications_count})
        combined_html = {
            'total_job_posts_html': total_job_posts_html,
            # 'applied_users_count_html': applied_users_count_html,
            'accepted_applications_count_html': accepted_applications_count_html,
            # 'rejected_applications_count_html': rejected_applications_count_html,
            # 'pending_applications_count_html': pending_applications_count_html,
        }

       
        return render(request, self.template_name, combined_html)

class DashboardView11(View):
    def get(self, request, *args, **kwargs):
        try:
            total_job_posts = JobPost.objects.filter(user=request.user).count()
            total_applications = Apply_Job.objects.filter(job__user=request.user).count()
            accepted_applications_count = Apply_Job.objects.filter(job__user=request.user, status='Accepted').count()
            rejected_applications_count = Apply_Job.objects.filter(job__user=request.user, status='Rejected').count()
            pending_applications_count = Apply_Job.objects.filter(job__user=request.user, status='Pending').count()
